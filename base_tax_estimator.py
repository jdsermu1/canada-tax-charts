...
class TaxEstimator:
    def __init__(self, limits, rates):
        ...
        if len(limits) != len(rates):
            raise exceptions.EstimatorError("limits and rates should have the same size")
        if limits[0] != 0:
            raise exceptions.EstimatorError("First lower limit should be 0 currently is " + limits[0])
